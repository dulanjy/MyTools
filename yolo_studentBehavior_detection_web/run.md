...
- Maven Wrapper 宸插唴缃紙鏃犻渶鍗曠嫭瀹夎 Maven锛?- Python Conda 鐜锛歚yolov8`锛堝凡瀛樺湪锛?- MySQL锛堢敤浜庡鍏?`student_behavior` 搴擄級
...
## 鏁版嵁搴撳鍏?- MySQL 鍒涘缓鏁版嵁搴撳苟瀵煎叆 SQL锛?  1. 鍒涘缓搴擄細`student_behavior`
  2. 瀵煎叆鏂囦欢锛歚yolo_studentBehavior_detection_web/student_behavior.sql`
- Spring Boot 榛樿杩炴帴锛?  - URL: `jdbc:mysql://localhost:3306/student_behavior`
  - 鐢ㄦ埛/瀵嗙爜锛歚root/123456`锛堝涓庝綘鏈湴涓嶅悓锛岃鍦?`yolo_studentBehavior_detection_web/yolo_studentBehavior_detection_springboot/src/main/resources/application.properties` 涓慨鏀癸級

## 鍚姩 Flask锛?000锛?鍦ㄤ粨搴撴牴鐩綍鎵ц锛?```powershell
(D:\Software\anaconda3\envs\yolo11) ; (conda activate yolo11)
cd /d D:\Software\yolo11\ultralytics-main\Web\yolo_studentBehavior_detection_web\yolo_studentBehavior_detection_flask
python .\main.py
```
鍚姩鎴愬姛鍚庯紝绔彛 5000 浼氬浜?LISTENING銆?
## 鍚姩 Spring Boot锛?999锛?鍦ㄦ柊鐨?PowerShell 绐楀彛鎵ц锛?```powershell
cd /d D:\Software\yolo11\ultralytics-main\Web\yolo_studentBehavior_detection_web\yolo_studentBehavior_detection_springboot
...
cd /d D:\Software\yolo11\ultralytics-main\Web\yolo_studentBehavior_detection_web\yolo_studentBehavior_detection_vue
npm run dev
```
