		...
		<div class="system-predict-padding layout-padding-auto layout-padding-view predict-view">
			<section class="control-panel">
				...
				<div class="action-row">
					...
					>
						<el-button type="info" plain class="action-btn">上传视频</el-button>
					</el-upload>
					<el-button type="primary" :loading="state.processing" class="action-btn" @click="upData">开始处理</el-button>
				</div>

				<p class="upload-tip">
					{{ state.form.inputVideo ? `已上传：${shortInputName}` : '请先上传待检测视频（MP4/AVI 等）' }}
				</p>
			</section>
			...
			<section class="workspace-grid">
				<section class="preview-panel">
					<div class="panel-head">
						<h3>视频检测结果</h3>
						<span>处理后实时显示检测帧</span>
					</div>
					<div class="media-stage">
						<img v-if="state.video_path" class="video-frame" :src="state.video_path" />
						<div v-else class="empty-state">
							<el-icon class="empty-icon"><VideoPlay /></el-icon>
							<p>上传视频并点击“开始处理”后显示结果</p>
						</div>
					</div>
				</section>
...
const CHART_INIT_MAX_RETRIES = 24;
...
const shortInputName = computed(() => {
	const value = String(state.form.inputVideo || '');
	if (!value) return '';
...
const parseMaybeJson = <T = any>(value: any, fallback: T): T => {
	...
	if (!txt || txt === 'undefined' || txt === 'null') return fallback;
	try {
...
const cancelPendingChartUpdate = () => {
...
const getChartContainers = () => {
	const trendEl = trendChartRef.value;
	const classEl = classChartRef.value;
	if (!trendEl || !classEl) return null;
	if (trendEl.clientWidth <= 0 || trendEl.clientHeight <= 0) return null;
	if (classEl.clientWidth <= 0 || classEl.clientHeight <= 0) return null;
	return { trendEl, classEl };
};
...
const updateCharts = (retry = 0) => {
	const containers = getChartContainers();
	if (!containers) {
		if (retry < CHART_INIT_MAX_RETRIES) {
			cancelPendingChartUpdate();
			chartUpdateRafId = requestAnimationFrame(() => {
				chartUpdateRafId = null;
				updateCharts(retry + 1);
			});
		}
		return;
	}
	const { trendEl, classEl } = containers;
	if (!trendChart) trendChart = echarts.init(trendEl);
	if (!classChart) classChart = echarts.init(classEl);
...
const upData = () => {
	...
	const queryParams = new URLSearchParams({
		...
		inputVideo: String(state.form.inputVideo || ''),
	...
	state.video_path = `${flaskBaseUrl}/predictVideo?${queryParams}`;
	ElMessage.success('开始处理，请稍候');
};
