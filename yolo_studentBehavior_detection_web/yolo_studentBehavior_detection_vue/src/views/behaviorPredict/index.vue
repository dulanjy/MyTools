...
const VISUALIZATION_INIT_MAX_RETRIES = 24;
let visualizationInitRafId: number | null = null;

const cancelPendingVisualizationInit = () => {
	if (visualizationInitRafId !== null) {
		cancelAnimationFrame(visualizationInitRafId);
		visualizationInitRafId = null;
	}
};

const scheduleVisualizationInit = (retry = 0) => {
	if (retry > VISUALIZATION_INIT_MAX_RETRIES) return;
	cancelPendingVisualizationInit();
	visualizationInitRafId = requestAnimationFrame(() => {
		visualizationInitRafId = null;
		initVisualizationCharts(retry);
	});
};

const getVisualizationChartContainers = () => [
...
const isContainerReady = (el: HTMLElement | null) => !!el && el.clientWidth > 0 && el.clientHeight > 0;
...
const resizeVisualizationCharts = () => {
	if (
		...
		if (analysisHasCharts.value) initVisualizationCharts();
		return;
	}
...
const initVisualizationCharts = (retry = 0) => {
	if (!analysisHasCharts.value) {
		disposeVisualizationCharts();
		return;
	}
	const chartContainers = getVisualizationChartContainers();
	const ready = chartContainers.every((el) => isContainerReady(el));
	if (!ready) {
		scheduleVisualizationInit(retry + 1);
		return;
	}

	disposeVisualizationCharts();
	classBarChart = echarts.init(chartContainers[0]!);
	classRoseChart = echarts.init(chartContainers[1]!);
	metricsRadarChart = echarts.init(chartContainers[2]!);
	metricsBarChart = echarts.init(chartContainers[3]!);
	spatialHeatmapChart = echarts.init(chartContainers[4]!);
	focusGaugeChart = echarts.init(chartContainers[5]!);
	classMixChart = echarts.init(chartContainers[6]!);
	spatialProfileChart = echarts.init(chartContainers[7]!);
	metricRingChart = echarts.init(chartContainers[8]!);
...
watch(
	[analysisHasCharts, analysisJson, result, analysisCounts],
	() => {
		nextTick(() => {
			if (!analysisHasCharts.value) {
				disposeVisualizationCharts();
...
onMounted(() => {
	getWeights();
	nextTick(() => initVisualizationCharts());
